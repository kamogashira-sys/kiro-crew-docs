...
`vX.Y.Z` 形式です。実在する安定版は本サイト作成時点（2026-08-22）で **v0.1.0／v0.1.1／v0.1.2／v0.1.3／v0.2.0／v0.3.0** の6件（GitHub Releases実測）。v0.2.0系のプレリリースは `-rc.N` サフィックスで区別されます（例: `v0.2.0-rc.1`）。v0.1.0〜v0.1.3系には`-insider.N`サフィックスのプレリリースも多数存在します（例: `v0.1.3-insider.2`）。**v0.3.0系のプレリリースはGitHub Releases実測で0件**（`v0.3.0`本体のみで`-rc.N`・`-insider.N`いずれも存在しません）。v0.4.0系は`-insider.N`サフィックスのプレリリースのみ存在し、安定版タグ・Releaseは本サイト作成時点で存在しません（本サイトの対応範囲外）。本サイトは安定版のみを扱う方針のため、いずれのプレリリースも本文の解説対象にはしていません。
...
CHANGELOG.mdの日付とGitHub Releaseの公開日は**一致しません**（実測で4版すべてで食い違い）。
