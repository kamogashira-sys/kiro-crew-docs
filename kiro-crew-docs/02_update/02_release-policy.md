...
`vX.Y.Z` 形式です。実在する安定版は本サイト作成時点で **v0.1.0／v0.1.1／v0.1.2／v0.1.3／v0.2.0** の5件（GitHub Releases実測）。プレリリースは `-rc.N` サフィックスで区別されます（例: `v0.2.0-rc.1`）。
...
- 日付が食い違う理由（CHANGELOG更新のタイミングとRelease公開のタイミングが異なるプロセスによるものと推測されるが、公式に明記された説明は見つかっていない）
