...
規約 (a)〜(i):
    ...
    (h) **存在しないCLIサブコマンドの記載**（Crew固有・新規）
        許可リストは公式cli-reference・README・`modules/cli.md`の3系統の和集合から生成
        （Phase 4で`modules/cli.md`を追加）。`DENIED_CLI_COMMANDS`に明示登録した語（いずれの
        系統にも一次情報で確認できないと確定した語）はerrorにする。
        ⚠️ **限界**: `DENIED_CLI_COMMANDS`は現時点で空集合であり、未登録の新規コマンドは
        「許可リスト外（要確認）」として件数表示されるのみでerrorにはならない
        （終了ステータスに影響しない）。これは一次情報側が先に更新された場合の誤検知を
        避けるための意図的な設計だが、逆に「許可リストに追加すべきだが未登録のまま」という
        脱落は検出できない（レビューで発見されたF-06参照）。許可リストの網羅性そのものは
        本規則では保証されない。
